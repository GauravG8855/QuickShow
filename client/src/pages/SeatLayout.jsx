...
const SeatLayout = () => {
  ...
  const checkout = () =>{
    ...
    if(selectedSeats.length === 0){
      return toast('Please select Seat')
    }
  ...
  const handleSeatClick = (seatId) => {
      ...
      if(!selectedSeats.includes(seatId) && selectedSeats.length > 4){
        return toast('Only select 5 seats at max')
      }
  ...
  const bookTickets = async () =>{
    try {
      ...
      if(!selectedTime || !selectedSeats.length) return toast.error('Please select time and seat.')
  ...
  return show ? (
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30  md:pt-50">
        ...
        <div className="mt-5 space-y-1">
          {show.dateTime[date].map((item)=>(
            <div key={item.time} onClick={() => setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ? "bg-primary text-white" : "hover:bg-primary/20"}`}>
              ...
              <p className="text-sm">{isoTimeFormat(item.time).split(',')[1]}</p>
            </div>
          ))}
        </div>
      </div >
      ...
      <div className=" relative flex-1 flex flex-col items-center max-md:mt-16 ">
            ...
            <button onClick={bookTickets} className="flex items-center gap-4 mt-20 px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95">
              Proceed to Checkout
